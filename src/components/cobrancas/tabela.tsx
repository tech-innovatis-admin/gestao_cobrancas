...
export const TabelaCobrancas = ({ rows, total, page, size, onAbrir }: { rows: Receivable[]; total: number; page: number; size: number; onAbrir: (id: string) => void }) => {
  ...
  const trunc = (v: string | null) => <span className="block truncate" title={v ?? ""}>{v ?? "—"}</span>;
  return (
    <div className="panel">
        ...
        <div className="max-h-[calc(100vh-330px)] overflow-auto">
          ...
          <TableBody>{rows.map((r) => (
            <TableRow key={r.id} className={cn("clicavel", r.is_overdue && "bg-danger-soft/40!", r.sync_status === "error" && "bg-danger-soft/60!")} onClick={() => onAbrir(r.project_id)}>
              ...
              <TableCell className="fix"><span className="inline-flex items-center gap-1">{fmtCompetencia(r.competence)}{r.legacy_consolidated && <BadgeConsolidado />}</span></TableCell>
              <TableCell>{r.hub}</TableCell><TableCell>{trunc(r.ministry_government)}</TableCell><TableCell>{trunc(r.institute)}</TableCell><TableCell>{trunc(r.foundation)}</TableCell>
              <TableCell><span className="flex min-w-0 items-center gap-1.5"><span className="truncate font-medium" title={r.project_name}>{r.project_name}</span>{r.provisional && <BadgeProvisorio />}</span></TableCell>
              ...
              <TableCell>{trunc(r.collection_status_label)}</TableCell><TableCell>{trunc(r.reason)}</TableCell><TableCell>{trunc(r.action)}</TableCell><TableCell>{trunc(r.responsible_name)}</TableCell>
