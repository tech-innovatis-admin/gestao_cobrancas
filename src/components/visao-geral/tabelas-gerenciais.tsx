...
export const TabelaGerencial = ({ titulo, tom, rows, vazio }: { titulo: string; tom: "danger" | "ok"; rows: Receivable[]; vazio: string }) => (
  <div className="panel overflow-hidden">
    <div className={cn("grid grid-cols-[1fr_auto_1fr] items-center px-4 py-2 text-white", tom === "danger" ? "bg-danger" : "bg-ok")}><span /><span className="col-start-2 text-center text-[13px] font-semibold uppercase tracking-wide">{titulo}</span><span className="num col-start-3 justify-self-end text-[12px] opacity-90">{rows.length}</span></div>
